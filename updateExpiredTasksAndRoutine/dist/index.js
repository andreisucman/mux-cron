...
async function run() {
    try {
        const expiredTasks = await doWithRetries(async () => db
            .collection("Task")
            .aggregate([
        ...
        const { modifiedCount } = await doWithRetries(async () => db
            .collection("Task")
            .updateMany({ status: "active", expiresAt: { $lte: new Date() } }, { $set: { status: "expired" } }));
        const { modifiedCount: modfiedRoutines } = await doWithRetries(async () => db
            .collection("Routine")
            .updateMany({ status: "active", lastDate: { $lte: new Date() } }, { $set: { status: "inactive" } }));
