...
async function handleAllocateReward(
  ...
  try {
    const updateUserOps: any = [];
    const updateUserStatsOps: any = [];
    const updateTotalStatsOps: any = [];
    ...
    for (const user of users) {
      ...
      if (followingUserId) {
        ...
        updateUserOps.push(updateUserOp);

        const today = new Date().toDateString();
        const updateUserStatsOp = {
          updateOne: {
            filter: { userId: new ObjectId(followingUserId), createdAt: today },
            update: {
              $inc: {
                totalPayable: dailyRewardShare,
              },
            },
          },
        };
        updateUserStatsOps.push(updateUserStatsOp);

        const updateTotalStatsOp = {
          updateOne: {
        ...
        updateTotalStatsOps.push(updateTotalStatsOp);
      }
    }

    if (updateUserOps.length > 0) {
      await doWithRetries(async () => {
        await db.collection("User").bulkWrite(updateUserOps);
      });
    }

    if (updateUserStatsOps.length > 0) {
      await doWithRetries(async () =>
        adminDb.collection("UserStats").bulkWrite(updateUserStatsOps)
      );

      await doWithRetries(async () =>
        adminDb.collection("TotalStats").bulkWrite(updateTotalStatsOps)
      );
    }
  } catch (err) {
    throw err;
