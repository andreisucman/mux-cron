...
export default async function getFinancialCalculations() {
    try {
        const latestTotalAnalyticsDoc = await doWithRetries(async () => adminDb
            ...
            .next());
        if (!latestTotalAnalyticsDoc)
            return;
        const { overview } = latestTotalAnalyticsDoc;
        const { user } = overview;
        const { totalUsers, totalRevenue, totalCost, totalReward, totalPayable, totalWithdrawn, } = user;
